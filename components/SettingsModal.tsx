...
const getSessionFromTime = (date: Date): TradeSession => {
  ...
  if (hour >= 8 && hour < 17) return TradeSession.NY;
  if (hour >= 2 && hour < 8) return TradeSession.LONDON;
      ...
      for (const exec of execs) {
          ...
          if (Math.abs(currentPosition) < 0.0001 && startTime && direction) {
              ...
              trades.push({
                  ...
                  direction: direction,
                  session: getSessionFromTime(startTime),
                  rating: 3,
      ...
      if ('showSaveFilePicker' in window) {
        try {
          ...
          if (err.name !== 'AbortError') {
             console.warn('File System API error, using fallback:', err);
             downloadFallback();
          }
        }
      } else {
